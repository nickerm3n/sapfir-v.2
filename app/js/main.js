...
(function($) {
    ...
    $("#pagepiling").pagepiling({
      ...
      afterLoad: function(anchorLink, index) {
        if ($(window).width() <= 414 || $(window).height() <= 747) {
          ...
          $(`section[data-anchor="${anchorLink}"]`)
  ...
  $(".slide-button").click(() => $.fn.pagepiling.moveSectionDown());
  $(".slide-up-button").click(() => $.fn.pagepiling.moveSectionUp());
  $(".slide-down-button").click(() => $.fn.pagepiling.moveSectionDown());
  ...
  const Menu = {
    ...
    activateMenu: function() {
      ...
      Menu.el.heroImageContent.toggleClass(
        "hero-image-content__text-wrapper--active"
      );
    }
  };
  ...
  function CreateLinks(number, path, parentNode) {
    ...
    for (let i = 2; i < number; i++) {
      let link = $(
        `<a href="./images/${path}/${i}.jpg" data-fancybox="${path}"></a>`
      );
